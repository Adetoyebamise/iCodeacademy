...
var RomanNumerals = function(){
   var structure = /^(M?)(M?)(M?)(?:(CM)|(CD)|(D?)(C?)(C?)(C?))(?:(XC)|(XL)|(L?)(X?)(X?)(X?))(?:(IX)|(IV)|(V?)(I?)(I?)(I?))$/,
       tokens = {
         'M': 1000,
         'CM': 900,
         'D': 500,
   ...
   function toRoman(i){
     ...
     return Object.keys(tokens).reduce( function(pair, token){
       var value = tokens[token],
           remainder = pair[1] % value,
           digit = (pair[1] - remainder) / value;
       return [ pair[0] + Array(digit + 1).join(token) , remainder ];
     }, ['', i] )[0];
   }
   function fromRoman(s){
     var matched = s.match(structure);
     if(!matched){
        throw new Error('not a valid roman numeral');
     }
     return matched.slice(1).reduce(function(i, token){
       return i + (tokens[token] || 0);
     }, 0);
   }
   return {
     toRoman: toRoman,
 ...
 const RomanNumerals = {
   toRoman: num => [...String(num)].reverse().map(digitToNumeral).reverse().join(''),
   fromRoman: str => [...str].map(numeralToDigit).reduce((sum, val) => sum + val)
}

const numeralToDigit = (numeral, index, arr) => {
   let number = getNumber(numeral),
       nextNumber = index < arr.length ? getNumber(arr[++index]) : 0;
       
   return nextNumber > number ? -number : number;
}

const digitToNumeral = (digit, position) => {
   const [one, five, ten] = getThreeSymbols(position);
   switch (Number(digit)) {
       case 0: 
       case 1:
       case 2:
...
const getNumber = numeral => {
   const index = symbols.indexOf(numeral),
       exponent = Math.floor(index / 2),
       offset = exponent * 2;
   
   return ((index - offset) * 4 + 1) * 10**exponent;
}

const getThreeSymbols = position => symbols.slice(position * 2, position * 2 + 3);
